      ...
      addProduct: (productData) => {
        ...
        set((currentState) => {
          ...
          return {
            ...
            // После создания продукта сохраняем текущее значение hasLoadedFromAPI
            // Если продуктов не было из API, но пользователь создал продукт,
            // hasLoadedFromAPI остается false, что позволит загрузить продукты из API при следующем визите
            // и объединить их с созданными пользователем продуктами
            hasLoadedFromAPI: currentState.hasLoadedFromAPI,
          };
        });
